...
if ( ! class_exists( 'Kirki' ) ) {
		...
		class Kirki_Installer_Section extends WP_Customize_Section {
			...
			protected function render() {
				...
				<div style="padding:10px 14px;">
					<?php if ( ! $installed ) : ?>
						<?php esc_attr_e( '
						Kirki Toolkit plugin is recommended to take advantage of this theme\'s features in the customizer.', 'redxunlite' ); ?>
						<a class="install-now button-primary button" data-slug="kirki" href="<?php echo esc_url_raw( $plugin_install_url ); ?>" aria-label="Install Kirki Toolkit now" data-name="Kirki Toolkit"><?php esc_html_e( 'Install Now', 'redxunlite' ); ?></a>
					<?php else : ?>
						<?php esc_attr_e( '
						Kirki Toolkit plugin is recommended to take advantage of this theme\'s features in the customizer.', 'redxunlite' ); ?>
						<a class="install-now button-secondary button" data-slug="kirki" href="<?php echo esc_url_raw( self_admin_url( 'plugins.php' ) ); ?>" aria-label="Activate Kirki Toolkit now" data-name="Kirki Toolkit"><?php esc_html_e( 'Activate Now', 'redxunlite' ); ?></a>
					<?php endif; ?>
				</div>
				<?php
			}
	...
	if ( ! function_exists( 'kirki_installer_register' ) ) {
		...
		function kirki_installer_register( $wp_customize ) {
			...
			$wp_customize->add_setting( 'kirki_installer_setting', array('sanitize_callback' => 'esc_url_raw',) );
